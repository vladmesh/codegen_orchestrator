...
class TestWorkerWrapperComponent:
    ...
    async def test_full_execution_flow(self, wrapper_config, fake_redis):
        ...
        broker_client.get_session.return_value = None

        wrapper = WorkerWrapper(config=wrapper_config, broker_client=broker_client)
        ...
        # Verify session captured from output and sent through the broker.
        broker_client.set_session.assert_awaited_once_with("test-session-123")
