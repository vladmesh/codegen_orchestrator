...
- В рамках brainstorm идёт обсуждение. Ассистент-ресерчер формирует Action Items.
...
- **Architect (Decomposition)**: В целевом процессе `/architect` разбивает крупные Story или сложные Brainstorm-отчёты напрямую в набор технических Tasks, присваивая им `repository_id` (мультирепо подход). Этот скилл работает на макроуровне (Story → Tasks), в отличие от `/plan` (микроуровень реализации одной Task).
