...
**Статусы:** `created` → `in_progress` → `deploying` → `completed` (также: `failed`, `archived`). `deploying` — deploy gate: story ждёт успешного деплоя перед завершением.
**Таблица:** `stories`
...
**Статусы:** `backlog` → `todo` → `in_dev` → `in_ci` → `testing` → `done` (также: `blocked`, `failed`, `cancelled`)
