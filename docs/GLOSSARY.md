...
**Статусы:** `not_deployed`, `running`, `stopped`, `down`, `degraded`
...
**Статусы:** `created` → `in_progress` → `deploying` → `completed` (также: `reopened`, `waiting_human_review`, `failed`, `archived`). `deploying` — deploy gate: story ждёт успешного деплоя перед завершением. `waiting_human_review` — developer agent сообщил о блокере; ожидание вмешательства админа. `reopened` — пользователь сообщил о проблеме с completed/failed story; architect пересматривает и создаёт fix-задачи.
**Таблица:** `stories`
...
`waiting_human_review` — developer agent сообщил о блокере через `POST localhost:9090/blocker`. Pipeline приостановлен до вмешательства админа (`POST /tasks/{id}/resume`).
