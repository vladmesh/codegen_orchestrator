...
**Статусы:** `created` → `in_progress` → `deploying` → `completed` (также: `waiting_human_review`, `failed`, `archived`). `deploying` — deploy gate: story ждёт успешного деплоя перед завершением. `waiting_human_review` — developer agent сообщил о блокере; ожидание вмешательства админа.
**Таблица:** `stories`
