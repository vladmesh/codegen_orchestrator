...
- **Status**: backlog
- **Brief**: Two parts:  1. **Ansible role**: Add Promtail to `services/infra-service/ansible/roles/monitoring/`. Promtail scrapes Docker containers with label `com.codegen.project_id`. Config template: push to orchestrator Loki at `<ORCHESTRATOR_HOST>:3100`. Add UFW rule for Promtail port if needed. Deploy v...

### #1032 Add com.codegen.project_id label to deployed containers
- **Priority**: MEDIUM
- **Plan**: —
- **Status**: backlog
- **Brief**: When deploying a user project, all containers must have Docker label `com.codegen.project_id=<uuid>`. This is how Promtail on prod servers knows which logs belong to which project.  Implementation: in the deploy pipeline (compose files generated by service-template or deploy.yml workflow), inject...
