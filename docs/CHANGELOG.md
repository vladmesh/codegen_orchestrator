...
### Added
- **Ansible role: qa_runner provisioning** (task-b6e972e4): New Ansible role that installs Node.js (NodeSource), Claude Code CLI, telethon+httpx on prod servers. Creates `/opt/qa-runner/` with ANTHROPIC_API_KEY env file and optional Telethon session. Included in `site.yml` and `provision_software.yml`. All idempotent. 14 unit tests.
