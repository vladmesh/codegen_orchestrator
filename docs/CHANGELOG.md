...
- Main CI pushes the 10 service images under the merge SHA and, only after a green gate, a
  `service-release:<sha>` digest marker (worker-chain protocol), so deploys can later pull a release.
