...
- **Dependency locks unified on `uv.lock` (P0 — kill the prod/test desync)**: there used to be four independent dependency resolutions — prod (`requirements.lock`), dev/CI (`uv.lock`, which was *gitignored*), service tests (`pip install .` from `pyproject` ranges), and the integration/e2e runners (hand-written, unpinned `requirements.txt`). They drifted hard (tests ran FastAPI 0.136 while prod shipped 0.127). Now `uv.lock` is the single source of truth and is committed; `make lock-deps` derives every `services/*/requirements.lock` from it via `uv export` (+ a new root `requirements-test.lock` exported from a pinned `test` dependency-group). `worker-manager` and `infra-service` prod Dockerfiles now install from `requirements.lock` instead of floating `pip install .`; every `Dockerfile.test` and the integration/e2e/template runners install pinned `requirements.lock` + `requirements-test.lock` (with `pip install --no-deps ./shared` so shared can't re-float its deps). `scaffolder` joined the uv workspace (locked, but not synced into the dev venv — its `tests/__init__.py` would otherwise shadow other services' namespace `tests` packages). CI now runs `uv sync --locked` and a `make lock-check` gate that fails if any `requirements.lock` drifts from `uv.lock`. Deleted the obsolete unpinned `docker/test/**/requirements*.txt`.
