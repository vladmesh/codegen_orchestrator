...
- The QA executor is a sandbox: its proxy opens the model backend, the deploy target (writes allowed, owner decision)
  and Telegram; it gets the QA Telethon identity only after a per-run proof, and no platform secret.
