...
- Every CI job has a `timeout-minutes` that covers the worst case of its bounded steps (every retry attempt at
  its bound, kill-after, backoff, margin; the CI contract sums it per leg), and each docker step a bound that
  marks `step-timeout`; a hung Buildx or image pull attempt is stopped and retried, and only the bound's own
  timer counts as a timeout. The publish job exposes its own marker. Stand bring-up, target and image steps
  are bounded.
...
- `mega-brief-package` also reads the deployed product's own package contract and job registry, so a
  hand-written imitation of the reminders capability is red instead of passing as the package route.
...
- Every QA executor attempt that produced output is retained under its own header, so a retry that
...
- The provisioner logs the play's QA-identity report as its own field, so it reaches an artifact even when
  later roles push it out of the play tail; the retrofit play now reports it under the same key.
- `qa_identity_unreadable` joins the QA blockers an operator may clear with `recheck-qa`, so naming the
  permission problem does not remove the recheck the same failure had as `server_unavailable`.
