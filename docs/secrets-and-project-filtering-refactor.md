...
4. **[DONE] Обновить ПО для обработки missing secrets**
   - Tool `save_project_secret` добавлен в deploy capability
   - `SecretResolverNode` читает секреты из `project_spec.config.secrets`
   - PO может собирать секреты от пользователя и сохранять через `save_project_secret`
