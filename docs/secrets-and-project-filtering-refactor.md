...
> Дата: 2025-12-29
> Статус: В работе (Итерация 0 завершена)
...
### Итерация 1: Базовая классификация секретов

**Цель:** Научить систему различать типы секретов без полной переделки DevOps.

**Задачи:**

1. **Добавить INFRA_SECRETS константу**
   - Файл: `services/langgraph/src/tools/activation.py`
   ```python
   INFRA_SECRETS = {
       "REDIS_URL", "DATABASE_URL", "ASYNC_DATABASE_URL",
       "POSTGRES_HOST", "POSTGRES_PORT", "POSTGRES_DB",
       "POSTGRES_USER", "POSTGRES_PASSWORD", "POSTGRES_REQUIRE_SSL",
       "APP_SECRET_KEY", "APP_NAME", "APP_ENV", "ENVIRONMENT",
       "BACKEND_API_URL", "DEBUG", "ENABLED_MODULES",
   }
   ```

2. **Фильтровать missing_secrets**
   - Файл: `services/langgraph/src/tools/activation.py`
   - В `inspect_repository` и `check_ready_to_deploy`:
   ```python
   user_required = [s for s in required_secrets if s not in INFRA_SECRETS]
   missing_secrets = [s for s in user_required if s not in existing_secrets]
   ```

3. **Обновить схему ответа**
   - Добавить `infra_secrets` и `user_secrets` в `RepositoryInspectionResult`

**Рекомендации к итерации 1:**
- Это временное решение (whitelist)
- Покрывает 90% случаев для service-template проектов
- Позволяет продолжать работу пока делается итерация 2

---

### Итерация 2: DevOps как LLMNode

**Цель:** Превратить DevOps в интеллектуальную ноду, способную анализировать код.

**Задачи:**

1. **Создать DevOps LLMNode**
...
3. **Написать system prompt для DevOps**
   ```
...
- Добавить fallback на whitelist если LLM не уверен

---

### Итерация 3: DevOps Subgraph с возвратом к ПО

**Цель:** Реализовать полноценный subgraph с механизмом возврата недостающих секретов к ПО.

**Задачи:**

1. **Создать DevOps subgraph**
...
4. **Обновить ПО для обработки missing secrets**
...
### Итерация 4: Пул ресурсов (Telegram боты)
...
1. **Не хардкодить whitelist в нескольких местах** — создать единый источник правды
   ```python
   # shared/secrets.py
   class SecretClassifier:
       ...
       @classmethod
       def classify(cls, key: str, context: dict) -> Literal["infra", "computed", "user"]:
           ...
   ```

2. **DevOps LLM prompt должен быть в БД** — как у других агентов, для возможности редактирования без деплоя

3. **Секреты не должны попадать в логи** — использовать `structlog` с фильтрацией sensitive полей
...
|----------|-----------|--------|--------|
| 0. Quick fixes | P0 (критично) | Low | High |
| 1. Базовая классификация | P1 | Low | Medium |
| 2. DevOps LLMNode | P2 | Medium | High |
| 3. DevOps Subgraph | P2 | High | High |
| 4. Пул ресурсов | P3 | Medium | Medium |

**Рекомендуемый порядок:** 0 → 1 → 2 → 3 → 4

Итерации 0 и 1 можно сделать быстро и получить значительное улучшение UX. Итерации 2-3 — более фундаментальные изменения, требующие больше времени.
