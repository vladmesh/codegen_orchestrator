...
- → backlog #1020: SystemConfig model + CRUD API + seed script + fail-fast validation (Phase 1)
- → backlog #1021: ConfigStore helper with TTL cache (Phase 1, blocked by #1020)
- → backlog #1022: Switch services from hardcoded constants to ConfigStore (Phase 1, blocked by #1021)
- → backlog #1025: Admin UI: Settings page (Phase 4, blocked by #1020)
- → backlog #1006: Decouple deploy worker from story lifecycle (Phase 2 prerequisite, already existed)
- → backlog #1023: Queue contracts: Optional story_id + action field in DeployMessage/QAMessage (Phase 2, blocked by #1006)
- → backlog #1024: Thin API endpoints for admin actions — 7 endpoints (Phase 3, blocked by #1023)
- → backlog #1026: Admin UI: action buttons on entity pages (Phase 4, blocked by #1024)
- Note: secrets editor merged into #1026 (part of action buttons)
