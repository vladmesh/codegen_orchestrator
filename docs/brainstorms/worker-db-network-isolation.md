...
id: bs-c2ff1cd2
status: triaged
title: "Worker DB & Network Isolation"
created_at: 2026-03-07T12:37:05.401161Z
---
...
## Ревизия 2026-07-12 (гриллинг роя)

Гриллинг актуализировал этот план. Решения зафиксированы стадиями 9-10 в
[codegen-stabilization-v1](../plans/codegen-stabilization-v1.md); здесь — что изменилось по сути.

1. **Курс: швы, не ферма.** Пользователей мало, железо не покупаем. Закладываем в архитектуру
   точки расширения, чтобы при первых 10 пользователях добавление хоста было конфигурацией.
2. **Вариант E пересмотрен: вместо центрального worker-manager с `DOCKER_HOST=ssh://` — реплика
   worker-manager на каждом хосте.** Все реплики читают общий `worker:commands` через consumer
   group (он уже есть, `consumer.py` даже содержит заготовку hostname в `consumer_name`). Хост
   забирает задачу, когда есть свободный слот; placement получается capacity-aware без
   центрального шедулера. Compose, workspace-кэш, GC и introspection остаются локальными
   операциями хоста, ничего не тащим через SSH.
3. **Git — источник правды для workspace, диск — кэш.** Воркеры и так пушат после тасок.
   Ensure-режим (clone + make setup) переезжает из scaffolder в worker-manager и выполняется
   перед спавном воркера на его хосте. `workspace_ready` меняет смысл на «scaffold запушен в git»,
   ставится один раз, GC-обвязка со сбросом флага отмирает. Scaffolder остаётся сервисом рождения
   проекта: GitHub-репо, секреты, copier из пиненного релиза темплейта, branch protection, первый
   push — ничего хостового.
4. **Честность пула.** Очередь общая для всех пользователей, воркер никому не принадлежит.
   Единица сериализации — проект (project lock остаётся). Против монополизации пула — cap M
   одновременных слотов на пользователя, проверка при взятии задачи.
5. **Изоляция: главный риск не общий пул, а креды внутри контейнера с агентом.** Сегодня туда
   инжектятся `REDIS_URL` оркестратора, `SECRETS_ENCRYPTION_KEY` и орг-токен GitHub
   (`manager.py:422-438`) — это нарушает secret isolation и лечится Stage 9 (общение только через
   worker-manager proxy, per-repo токены). Multi-host ферма изоляцию улучшает: blast radius
   ограничен хостом фермы.
6. **Варианты F-J остаются в силе как подложки.** «Хост» — это абстракция: любая машина с Docker
   и репликой worker-manager. Kata/Firecracker (H) и elastic cloud VMs (F) — в бэклоге с
   триггерами (#1050, #1051). K8s и Swarm не рассматриваем.
7. `scaffold_phase.py` в worker-manager — мёртвый дубликат старого пути скаффолда, на удаление
   (Stage 4 плана стабилизации).
