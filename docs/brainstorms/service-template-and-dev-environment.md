...
## Пересмотренный pipeline: Scaffolder → Developer

### Текущий pipeline

```
PO → trigger_engineering → Scaffolder (copier copy) → Developer (пишет всё) → Tester
```

Scaffolder — отдельный сервис на `scaffolder:queue`. Делает только `copier copy`. Developer получает пустой скелет и сам решает про модели, эндпоинты, события. Контрактов нет — каждый сервис в вакууме.

### Проблема: кто проектирует контракты?

С contract-first подходом кто-то должен написать спеку (models.yaml, events.yaml, services.yaml) до того, как scaffolder сгенерирует код. Это архитектурное решение, не механическая работа.

**PO для этого не подходит.** PO — координатор, не архитектор. Если засунуть в него проектирование контрактов, промпт раздуется документацией service-template, и миграция на ReactAgent станет невозможной.

### Решение: TaskAssessor + опциональный Architect

**TaskAssessor** — LLM tool, вызываемый PO. Читает текущее состояние проекта (какие сервисы, какие события, какие спеки), оценивает сложность задачи, подсказывает PO какие уточняющие вопросы задать пользователю. Один LLM-вызов, копейки.

Зачем нужен: пользователь может не знать, какие модули уже есть в проекте (оркестратором пользуются люди, не читающие код). PO тоже не должен разбираться в деталях repo. TaskAssessor смотрит в проект и возвращает структурированную оценку.

**Architect node** — LLM-нода, вызывается только для сложных задач. Проектирует контракты (spec diff), передаёт scaffolder'у. Одноразовый вызов, не контейнер.

### Новый pipeline

```
PO → assess_task (TaskAssessor tool)
   → уточняющие вопросы пользователю (если нужно)
   → trigger_engineering(complexity, ...)
          │
          ├── simple ("добавь хендлер /start в tg_bot")
          │     → Developer → Tester
          │     Контракты не затронуты, спеки не нужны.
          │
          └── complex ("добавь notifications-сервис")
                → Architect node (LLM, проектирует spec diff)
                → Scaffolder node (mechanical, генерирует из спеки)
                → Developer → Tester
```

### Scaffolder: отдельный сервис → нода в subgraph

Scaffolder как отдельный сервис на очереди — артефакт старой архитектуры. С contract-first он получает спеку из предыдущей ноды (Architect) через graph state. Mechanical node внутри engineering subgraph: проще передача данных, retry из коробки, LangSmith tracing.

Должен уметь два режима: "создать проект с нуля" (copier copy + генерация) и "добавить сервис в существующий проект" (генерация нового сервиса + обновление shared-контрактов).

### Эскалация при неожиданной сложности

Developer в процессе работы обнаруживает, что задача сложнее (нужен новый event, нужен новый сервис) → `request_help` → pipeline эскалирует через Architect. Совпадает с правилом из CLAUDE.md: "если нужно менять shared/contracts — STOP".
