...
### Phase 3: Langfuse + LLM tracing (in progress)
...
- **Nginx proxy**: `/langfuse/` → `langfuse-web:3000` (без base path — `NEXT_PUBLIC_BASE_PATH` требует сборки из исходников). Полноценный UI на `:3002` напрямую.
- **Trace → Task linking**: отдельная задача после базовой интеграции (передавать task_id как metadata, сохранять trace_id в БД)

**Подзадачи:**
1. ✅ Langfuse infra — docker-compose (langfuse-web + langfuse-worker + ClickHouse + MinIO), nginx proxy, env vars, init script — #task-a51fb1cf
2. LangChain callback integration — env-var drop-in, проверка что трейсы идут — #task-300f55e6
3. Admin SPA — страница LLM Tracing (iframe Langfuse) — #task-df069084
4. Trace-task linking — per-task trace ID, ссылка в UI (Phase 3b)
...
- `NEXT_PUBLIC_BASE_PATH` — build-time переменная, prebuilt образ её не поддерживает. Для sub-path нужна сборка из исходников. Пока: UI на порту 3002 + convenience proxy `/langfuse/` (навигация внутри Langfuse ломается).
...
- ✅ ~~#task-a51fb1cf: "Langfuse v3 infra — docker-compose + ClickHouse + MinIO + nginx proxy"~~ → done (включая MinIO, т.к. S3 обязателен)
- → #task-300f55e6: "LangChain → Langfuse tracing integration (env-var drop-in)"
- → #task-df069084: "Admin SPA — LLM Tracing page (Langfuse iframe)"
