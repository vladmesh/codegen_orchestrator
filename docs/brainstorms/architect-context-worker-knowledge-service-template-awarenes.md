...
id: bs-d302b6a1
status: done
title: "Architect Context & Worker Knowledge — Service Template Awareness"
created_at: 2026-03-09T14:39:00.124312Z
---
...
Created as tasks in story `story-aa83f576` (Product decomposition + Architect node):

1. **task-60d92940**: Create scaffolder microservice
   - scaffold:queue consumer, copier + make setup + git push, tree to DB
   - Includes scheduler wiring (new repo → scaffold, scaffolded → architect)
2. **task-2378004c**: Architect receives tree + specs, creates tasks for diff
   - blocked_by task-60d92940
   - New tool (tree from DB), prompt rewrite, auto CI task append
3. **task-c6509602**: Worker-manager mounts workspace volume by repo_id
   - blocked_by task-2378004c
   - Remove scaffold from worker-manager, mount /data/workspaces/{repo_id}/

Full pipeline spec: [docs/PIPELINE_V2.md](../PIPELINE_V2.md)

### Future ideas (not tasked yet)
