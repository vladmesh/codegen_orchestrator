...
- → backlog #1011: Provisioning: install node_exporter + cadvisor + UFW rules — Phase 1
- → backlog #1012: Prometheus text format parser for node_exporter + cadvisor metrics — Phase 1
- → backlog #1013: Extend Server model with health metrics + metrics history table — Phase 1
- → backlog #1014: Implement health_checker worker (HTTP polling + auto-incidents + alerts) — Phase 1
- → backlog #1015: Admin UI: extended server health dashboard with per-container view + charts — Phase 1
- → backlog #1019: HTTP health prober for deployed applications + SSL expiry check — Phase 2
- → backlog #1016: Admin UI: application health status and response times — Phase 2
- → backlog #1017: Container drift detection via cadvisor (orphans/ghosts in health_checker) — Phase 3
- → backlog #1018: Daily SSH job: filesystem drift check + docker prune — Phase 3
- → idea: "Prometheus migration path when server count > 10" — future (added to docs/ideas.md)
