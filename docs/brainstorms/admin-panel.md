...
- **Разбивка по пользователям**: сейчас все трейсы в одной куче. Нужно передавать `user_id` (telegram_id) в Langfuse metadata через callback handler, чтобы фильтровать трейсы по юзеру
- **Разбивка по проектам**: передавать `project_id` как session/tag, чтобы видеть все LLM-вызовы в рамках одного проекта
- **Разбивка по нодам (агентам)**: ПО и Архитектор — отдельные агенты с разными паттернами использования. Нужно тегировать трейсы типом агента (`po`, `architect`, `engineering`, `deploy`), чтобы анализировать расход отдельно
- **Trace → Task linking**: передавать `task_id` в metadata, сохранять `trace_id` в БД, показывать ссылку на трейс на странице таска в админке

**Только после настройки** — думать о том, чтобы подёргать отдельные данные из Langfuse API в свою админку (стоимость по проекту, последние трейсы на странице юзера и т.д.)
