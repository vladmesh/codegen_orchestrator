...
Date: 2026-09-24  
Audited branch: main at 1b41f21e (after PR #597, sprint:1460), deployed to production 2026-09-24T12:56Z as an immutable release  
Previous refresh: 2026-09-23 evening, through a4790772 (after PR #588)  
...
- PRs #589-#597 (sprint:1460): `Publish Service Image Release` in CI on main (#589), pull-only `deploy.yml` with a `revision` input and one `Switch` step (#590), production and stand compose without source mounts (#591), lock-first installs with a CI lock-drift check (#592), content-keyed worker releases with candidate builds before the gate and the marker after it (#593), GHA layer cache, path-filtered parallel image imports and lint-only gating of heavy jobs (#594), the stand pulling the tested release with background worker pulls and a warm venv (#595), the stand sweep's API endpoint (#596), and a cleanup step that tolerates missing images and never fails a deploy (#597).
...
The repository is healthier than the original audit snapshot, and since sprint:1460 what runs in production is byte-for-byte what CI tested: an immutable release pulled by digest, with rollback as a tag. A large amount of transition residue has been removed, and recent work consistently moves failure handling toward typed outcomes, durable evidence and explicit recovery.
