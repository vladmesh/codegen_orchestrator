...
| Task create/update/event/status | `shared/contracts/dto/task.py` | `schemas/task.py`, `routers/tasks.py` | `dispatch_admitted` is durable scheduler authority. Brief-backed tasks created before coverage admission begin false even when their status is `todo`; the scheduler consumes only admitted tasks, so a raw status write or Story transition cannot release work. Tasks created after durable admission, including CI and QA repairs, are admitted immediately. |
