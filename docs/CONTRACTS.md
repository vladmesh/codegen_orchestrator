...
| Surface / model family | Canonical source | API exposure / owner | Non-type invariant |
|---|---|---|---|
| Product Brief and requirement coverage | `shared/contracts/dto/product_brief.py` | `routers/product_briefs.py` | A Product Brief is a versioned immutable record after confirmation. It holds intended users, languages, source wording and stable must-requirement IDs plus non-secret typed initial settings. Every new product Story binds one confirmed brief. The architect claims one durable planning attempt before writing its plan; the heartbeat fences supervisor retry accounting, and a retry owns a distinct attempt after the old owner ends or expires. The architect records each requirement's task/acceptance coverage or returned reason. `POST /product-briefs/{id}/admit` is the sole idempotent release operation: it verifies every stable ID and atomically admits only that Story's current planning-attempt tasks. An `incomplete` result returns the missing IDs without pretending admission succeeded. |
| Story create/update/status | `shared/contracts/dto/story.py` | `schemas/story.py`, `routers/stories.py` | `StoryCreate.type` is explicit. Product requires a confirmed brief; technical and fix work must name `technical` and cannot bind one. Owner notifications and QA handoff are durable story lifecycle state. |
| Task create/update/event/status | `shared/contracts/dto/task.py` | `schemas/task.py`, `routers/tasks.py` | `dispatch_admitted` is durable scheduler authority. Brief-backed tasks created before coverage admission begin false even when their status is `todo`; they carry the active durable planning-attempt ID, and the scheduler consumes only admitted tasks. Re-parenting recomputes this authority and cannot inherit a prior Story's admission. A raw status write or Story transition cannot release work. Tasks created after durable admission, including CI and QA repairs, are admitted immediately. |
