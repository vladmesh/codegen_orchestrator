...
| Task create/update/event/status | `shared/contracts/dto/task.py` | `schemas/task.py`, `routers/tasks.py` | `dispatch_admitted` is durable scheduler authority. Brief-backed architect tasks begin false even when their status is `todo`; the scheduler consumes only admitted tasks, so a raw status write or Story transition cannot release work. |
