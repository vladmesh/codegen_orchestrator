...
### US1: Простой бот по запросу ✨ **ПРИОРИТЕТ**
> Пользователь: "Сделай бота погоды" → Бот задаёт вопросы → Через 10 мин готовый бот

**Статус**: 🔶 ~70% готово
- ✅ Telegram → PO Agent → ответ
- ✅ PO может создать проект через API
- ⚠️ Engineering pipeline (spawn worker) — не тестирован E2E
- ❌ Автоматический деплой не проверен
...
- [ ] E2E тест: создание проекта через Telegram
- [ ] E2E тест: деплой проекта
