...
- **Current Phase**: Sprint 002 Phase 4 (silent failures → fail-fast).
  B3 incident journal reconciliation and B4 secret-resolver are complete; the swallow-list remains.
...
| 4 | Тихие ошибки → fail-fast (B3, B4, swallow-list) | Active: B3 incident journal and B4 secret-resolver complete; swallow-list remains |
...
| B4 secret resolver fail-fast (`codegen_orchestrator-473`) | COMPLETE | Resolver validates project context, allocations and repository metadata before deploy, rejects unknown computed values, and propagates secret-persistence failures through the deploy error path. Remaining Phase 4 swallow-list items stay open. |
