...
- [ ] **Iteration 7: Self-hosted Docker Registry + Caddy TLS** ← next
...
Self-hosted Docker Registry + Caddy TLS. Заменяет GHCR собственным registry в compose оркестратора.
Caddy попутно закрывает webhook endpoint через HTTPS.
Подробный план: [deploy-architecture.md](./plans/deploy-architecture.md#iteration-7-self-hosted-docker-registry--caddy-tls).
