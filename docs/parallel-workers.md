...
1. **Network setup**:
   ...
   opens their model backend, their deploy target's public URL and Telegram
   (direct application writes stay forbidden by QA policy and the runner's
   write guard, not by the network), and everything SSH-based goes through
   their per-run capability endpoint.
