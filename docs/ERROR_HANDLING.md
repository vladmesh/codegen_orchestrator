...
- **A consumer that dispatches concurrently has to renew a lease.** Most consumers process an entry inline, so an entry is only pending while the read loop is inside the handler and `pending_timeout_ms` alone tells a stuck entry from a live one. The PO consumer does not: it hands each entry to an `asyncio.Task` and ACKs in that task's `finally`, so an entry is legitimately pending for as long as the graph runs, and a sweep at `min_idle_time = pending_timeout_ms` — its own or another PO's — would hand back work that is still running. PO answers that with an explicit renewal and no second mechanism: `_renew_in_flight_leases` runs `XCLAIM {stream} {group} {consumer} 0 <ids> JUSTID` over the ids currently in flight every `LEASE_RENEWAL_INTERVAL_MS` (`PEL_TIMEOUT_MS // 3`), which resets their idle time without taking a body back and without touching the delivery count. It has to be `XCLAIM` and not the sweep: `XAUTOCLAIM` only ever returns entries whose idle time has *already* reached `min_idle_time`, so a sweep could refresh an entry no sooner than the moment every other consumer is allowed to claim it — a race, not a lease. Liveness is therefore a sign of work, not a reading of the clock, and it fails in the safe direction: the renewal is a task on the same event loop as the processing and reads the in-flight set on every turn, so a dead process, a jammed loop, or a task that ended (including one whose ACK raised) all stop it, and one `PEL_TIMEOUT_MS` later the entry is claimable by another PO. The read loop additionally keeps its in-flight ids and refuses to dispatch one twice, which covers a renewal that was late.
