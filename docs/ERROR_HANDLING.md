...
its plan came back `incomplete`, so recovery usually starts at the next
supervisor pass rather than after the heartbeat timeout — and a planner that
died without giving anything back is exactly the stale-heartbeat case above.
...
- **A consumer that dispatches concurrently has to recognise its own work coming back.** Most consumers process an entry inline, so an entry is only pending while the read loop is inside the handler and `pending_timeout_ms` alone tells a stuck entry from a live one. The PO consumer does not: it hands each entry to an `asyncio.Task` and ACKs in that task's `finally`, so an entry is legitimately pending for as long as the graph runs, and its own sweep — at `min_idle_time = pending_timeout_ms`, like everyone's — finds it idle past the bar and hands it back. PO keeps the ids it has in flight and does not dispatch one twice, so a redelivery to this process never starts a second `_process_message`. The id is dropped when the task ends, success or failure, which is after the ACK attempt: an entry whose ACK raised has to be allowed to age until a sweep reclaims it, because nothing is working on it any more.
