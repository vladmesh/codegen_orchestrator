...
> **Примечание:** Узел «Завхоз» (`nodes/zavhoz.py`) существует в коде, но **НЕ подключён** в LangGraph граф. Ресурсы выделяются через DevOps Subgraph. Этот документ описывает паттерн изоляции секретов, используемый в системе.
