...
1. [ ] Add `user_name` to POUserMessage and telegram bot
...
2. [ ] Inject user context into PO graph invocation
...
3. [ ] Add `hint` parameter to `set_project_secret` tool
...
4. [ ] Update PO system prompt with env hints + access control question
...
5. [ ] Inject env_hints into Developer Worker TASK.md
...
6. [ ] Integration test: full env_hints flow
