...
4. **Автоматический и Фоновый Cleanup (Garbage Collector)**:
   ...
   - ✅ **DONE**: Периодическая GC-джоба в `worker-manager` (каждые 30 мин) чистит осиротевшие контейнеры, сети `dev_proj_*` и воркспейсы. Реализовано в `WorkerManager.garbage_collect_orphaned_resources()` — сверяет Docker/FS ресурсы с `worker:status:*` ключами в Redis.
