...
1. [ ] Add `limit` query param to list work items endpoint
...
2. [ ] Add `sort` query param (optional: by priority or created_at)
...
3. [ ] Rewrite `/next` SKILL.md to use API
...
4. [ ] Add `GET /api/work-items/by-tag/{tag}` lookup endpoint
   - **Input**: `services/api/src/routers/work_items.py`
...
5. [ ] Service test: `/next` flow via API
