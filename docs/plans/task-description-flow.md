...
1. [x] Persist description to DB in `trigger_engineering`
...
2. [x] Use `feature_description` as fallback in `_build_create_task`
...
3. [x] Update PO prompt to pass detailed_spec in create_project
...
4. [x] Update brainstorm status and verify end-to-end
...
- Step 2 tests were added to existing `test_developer_node.py` (new `TestCreateTaskDetailedSpecFallback` class) instead of a new `tests/unit/nodes/test_developer.py` as the plan suggested — file already existed with developer node tests.
- Step 4: used `make test-unit` (all services) instead of `make test-langgraph-unit` (target doesn't exist). All 146 tests across 9 suites passed.
- Brainstorm was already marked `done` before implementation started.
