...
1. [x] Add unique constraint on `(server_handle, port)` via Alembic migration
...
2. [x] Add atomic allocate-or-next endpoint to API
...
3. [x] Add API client method and update allocator to use atomic endpoint
...
4. [x] Update PO tool `get_next_available_port` to use atomic endpoint
...
5. [x] Integration test: concurrent allocation race
...
- Steps 1+2 committed together (tightly coupled: constraint + endpoint)
- Steps 3+4 committed together (allocator update + dead code removal in one pass)
- Step 4: `ports.py` was entirely dead code (never imported outside `__init__.py`) — removed file entirely instead of updating tools. Also removed `PortAllocationResult` schema.
- No separate unit test for step 4 (removal = verified by lint + existing tests passing)
