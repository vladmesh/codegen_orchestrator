...
1. [x] Add `duckduckgo-search` dependency
...
3. [x] Update system prompt with search guidance
...
- **Step 2**: Used sync `DDGS().text()` instead of `AsyncDDGS().atext()` — duckduckgo-search v8 removed `AsyncDDGS`, the sync `DDGS` class is the only API. Tool is sync (not `async def`), LangGraph handles it fine.
- **Steps 1-3 combined into single commit** — all changes were small and tightly coupled.
