...
**Предусловие**: в целевом репозитории (например `test-scaffold`) должен существовать `deploy.yml` workflow. Если его нет — нужно создать вручную или через service-template.

```bash
make test-unit  # все тесты зелёные
make test-langgraph-unit  # все тесты зелёные (новые тесты deployer)
```

Ручная проверка:
1. Создать проект, пройти scaffolding + engineering
2. Тригернуть деплой через Telegram
3. **Ожидаемые логи langgraph:**
   - `secret_resolver_start`
   - `secret_resolver_complete resolved_count=N missing_user_count=0`
   - `deployer_start project_id=...`
   - `ci_secrets_configured owner=... repo=... secrets_count=6` (DOTENV + 5 infra)
   - `workflow_dispatch_triggered workflow_file=deploy.yml`
   - `waiting_for_workflow owner=... repo=... workflow_file=deploy.yml`
   - `workflow_completed run_id=... conclusion=success`
   - `service_deployment_record_created`
   - `deployer_complete status=success deployed_url=http://...`
4. В GitHub → repo → Actions → `deploy.yml` — последний run зелёный
5. SSH на сервер: `docker compose -f compose.base.yml -f compose.prod.yml ps` → все контейнеры running
6. В БД: `service_deployments` запись с `deployed_sha` != NULL
...
- `docs/NODES.md` — обновить секцию DevOps subgraph (убрать Ansible, описать GitHub Actions flow)
- `docs/CONTRACTS.md` — убрать `ansible:deploy:queue`
- `CLAUDE.md` — убрать infra-service из описания deploy flow, обновить architecture diagram
