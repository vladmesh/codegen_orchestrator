...
def mock_settings():
    ...
    with patch("src.tools.base.get_settings") as mock:
        settings = AsyncMock()
        settings.api_url = "http://api:8000/api"
...
async def test_api_client_base_url(mock_settings):
    ...
    assert client.base_url == "http://api:8000/api"
...
async def test_api_client_get_request(mock_settings):
    ...
    with patch("httpx.AsyncClient") as mock_client_class:
        ...
        assert call_kwargs["base_url"] == "http://api:8000/api"
...
async def test_api_client_post_request(mock_settings):
    ...
    with patch("httpx.AsyncClient") as mock_client_class:
        ...
        mock_client.post.assert_called_once_with("/projects/", json={"name": "test"})
