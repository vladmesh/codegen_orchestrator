...
that combination's own window and under its own name, and a combination that
emitted none says so instead of inheriting another's.
"""
...
def test_evidence_is_collected_within_the_combination_that_produced_it():
    """A combination cannot borrow the artifact of another one."""
    script = _script("matrix")
    selector = '-name "run-evidence-worker-${worker_agent}-qa-${qa_agent}-*.json"'
    assert selector in script
    assert '-newermt "@$started"' in script[script.index(selector) :]
...
def test_evidence_is_collected_before_the_matrix_cleans_up():
    ...
    script = _script("matrix")
    assert script.index("run-evidence-worker-") < script.rindex("cleanup_matrix")
