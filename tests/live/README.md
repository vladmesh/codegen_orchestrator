...
absence. Every named suite also compares the deploy Run's image references with the merged SHA before it
spends a QA attempt, so a deployment running an older image fails as a deploy defect rather than
as a product one. The LLM suites additionally prove selected executor wiring; they do not yet claim
...
| Deployed artifact identity | the deploy Run's image references, tagged with the merged SHA, read before any QA attempt | all named suites |
