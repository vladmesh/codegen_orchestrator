...
they are the ledger in `shared/stand_deadlines.py` (`NOOP_LIFECYCLE_WAITS`), the cap is derived from
it, and the runner spends that cap — so the three cannot drift apart the way they had when the
scaffold bound stopped being a constant. The ledger sums to 110m40s and the 125-minute cap leaves
14m20s for manifest-owned teardown and diagnostics — at least the 11m40s reserve the ledger
requires of it, which is checked where both are defined.

It has three parts. The **first story** spends 53m20s (`240 + 840 + 60 + 420 + 420 + 120 + 320 + 300
+ 180 + 180 + 120` seconds): scaffold; two ordered noop Tasks; Story aggregation; deploy Run; deploy;
the typed deploy outcome; a bounded public health probe (up to two 30-second paths per attempt);
deterministic QA; completed-story and durable PO delivery; the exact deployment record. The **second
story on the same project** spends 47m20s (`420 + 60 + 420 + 540 + 420 + 320 + 300 + 180 + 180`):
one noop Task and its Story aggregation; its merged deploy Run; the typed deploy outcome, which for a
second story has to cover the deploy itself, because the application is already `running` from the
first story's deploy and stays terminal throughout a redeploy — the Run is the fact, not the status;
the application's own terminal status once that Run has settled; the health probe; QA; completed-story
and PO delivery. **Teardown** spends 10m: undeploy Run, terminal application and port-allocation
release. The LLM pipeline
