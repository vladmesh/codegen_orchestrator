...
Exercises the entire path from project creation to a live /health response:
  ...
  5. All tasks done → dispatcher creates PR story/{id} → main (no GitHub auto-merge)
  6. CI runs on PR → green → PR poller writes registry secrets, merges → deploy:queue
    ...
    *,
    ...
    async with (
        ...
        async with owner_client as api:
            ...
            # conversation row, from the phase before it. It is taken for the
            # actor whose chat id the PO tools will compose their thread from —
            # this run's own user, so the thread is the run's too.
