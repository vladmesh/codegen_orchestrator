...
from src.subgraphs.devops.nodes import _setup_ci_secrets as setup_ci_secrets
...
class TestSetupCISecrets:
    ...
    async def test_setup_ci_secrets_success(self, tmp_path):
        ...
        with patch("src.subgraphs.devops.nodes.SSH_KEY_PATH", str(ssh_key_file)):
    ...
    async def test_setup_ci_secrets_ssh_key_not_found(self):
        ...
        with patch("src.subgraphs.devops.nodes.SSH_KEY_PATH", "/nonexistent/path/id_ed25519"):
            result = await setup_ci_secrets(
                github_client=mock_github_client,
                owner="test-org",
                repo="test-repo",
