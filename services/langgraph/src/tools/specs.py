...
@tool
async def create_spec_md(
    repo_full_name: Annotated[str, "Full repository name (org/repo)"],
    project_name: Annotated[str, "Project name/title"],
    description: Annotated[str, "Detailed project description"],
    requirements: Annotated[list[str], "List of functional requirements"],
    tech_stack: Annotated[list[str], "Technology stack and tools"],
    architecture: Annotated[str, "Description of architecture and components"],
) -> str:
    """Create SPEC.md file in the repository (Human Readable).

    Generates a beautifully formatted Markdown specification document.
    Use this alongside create_project_spec_yaml to document the project
    ...
    try:
        ...
        md_lines = [f"# {project_name}", "", f"{description}", "", "## Requirements", ""]

        for req in requirements:
            md_lines.append(f"- {req}")

        md_lines.extend(["", "## Tech Stack", ""])
        ...
        md_lines.extend(
            [
                ...
                "",
                "---",
                "*Generated by Codegen Orchestrator Analyst*",
            ]
        )
        ...
        await client.create_or_update_file(
            ...
            path="SPEC.md",
            content=content,
            message="Create SPEC.md project specification",
            branch="main",
        )

        return "SPEC.md created successfully."
    except Exception as e:
        return f"Failed to create SPEC.md: {str(e)}"
