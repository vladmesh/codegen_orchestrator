...
Topology:
    START -> env_contract_loader -> secret_resolver -> readiness_check
                                                      |
...
from .env_contract_loader import load_environment_contract
...
def route_after_environment_contract(state: DevOpsState) -> str:
    """Route after loading the environment contract."""
    if state.get("errors"):
        return END
...
def create_devops_subgraph() -> Any:
    ...
    graph.add_node("env_contract_loader", load_environment_contract)
    ...
    graph.add_edge("resource_allocator", "env_contract_loader")

    graph.add_conditional_edges(
        "env_contract_loader",
        route_after_environment_contract,
        {
