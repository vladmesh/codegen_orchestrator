...
"""
    ...
    try:
        ...
        stdout_brief = process.stdout[:1000] + "..." if len(process.stdout) > 1000 else process.stdout
...
async def reinstall_and_provision(
    ...
    try:
        ...
        if not password:
            ...
            logger.warning("Could not extract password from reinstall result. Attempting explicit password reset...")
        ...
        await update_server_labels_in_db(server_handle, {"provisioning_phase": "software_installation"})
...
async def run(state: dict) -> dict:
    ...
    if state.get("force_reinstall") or server_status == "force_rebuild":
         logger.info(f"Force reinstall requested for {server_handle} or server status is force_rebuild.")
         use_reinstall = True
    ...
    if use_reinstall:
        ...
        if success:
            return await handle_provisioning_success(server_handle, server_ip, provisioning_attempts, is_recovery, " (Reinstalled)")
        else:
            await update_server_status_in_db(server_handle, "error")
    ...
    # ===== EXISTING ACCESS PATH (SKIP REINSTALL) =====
        ...
        if success_soft:
            await update_server_labels_in_db(server_handle, {"provisioning_phase": "complete"})
            return await handle_provisioning_success(server_handle, server_ip, provisioning_attempts, is_recovery, " (Retried)")
        else:
            await update_server_status_in_db(server_handle, "error")
