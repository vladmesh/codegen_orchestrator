...
class TestPollOnce:
    @pytest.mark.asyncio
    async def test_fires_due_reminder(self, mock_redis):
        reminder = _make_reminder()
        mock_redis.zrangebyscore.return_value = [reminder]

        fired = await _poll_once(mock_redis)
    ...
    @pytest.mark.asyncio
    async def test_ignores_future_reminders(self, mock_redis):
    ...
    @pytest.mark.asyncio
    async def test_fires_multiple_reminders(self, mock_redis):
        r1 = _make_reminder(user_id="user-1", text="check task 1")
        r2 = _make_reminder(user_id="user-2", text="check task 2")
        mock_redis.zrangebyscore.return_value = [r1, r2]

        fired = await _poll_once(mock_redis)

        assert fired == 2  # noqa: PLR2004
        assert mock_redis.xadd.call_count == 2  # noqa: PLR2004
        assert mock_redis.zrem.call_count == 2  # noqa: PLR2004

    @pytest.mark.asyncio
    async def test_removes_fired_reminder(self, mock_redis):
        reminder = _make_reminder()
        mock_redis.zrangebyscore.return_value = [reminder]

        await _poll_once(mock_redis)

        mock_redis.zrem.assert_called_once_with(PO_REMINDERS_KEY, reminder)

    @pytest.mark.asyncio
    async def test_continues_on_parse_error(self, mock_redis):
        """Invalid JSON should be removed and not block other reminders."""
        bad_entry = "not-valid-json"
        good_reminder = _make_reminder()
        mock_redis.zrangebyscore.return_value = [bad_entry, good_reminder]
    ...
    @pytest.mark.asyncio
    async def test_queries_correct_score_range(self, mock_redis):
        ...
        mock_redis.zrangebyscore.assert_called_once_with(PO_REMINDERS_KEY, 0, 1000.0)
...
class TestRunReminderPoller:
    ...
    async def test_graceful_shutdown(self, mock_redis):
        ...
        mock_redis.zrangebyscore.side_effect = asyncio.CancelledError()

        # Should not raise — exits cleanly
        await run_reminder_poller(mock_redis)

    @pytest.mark.asyncio
    async def test_continues_on_error(self, mock_redis):
        ...
        mock_redis.zrangebyscore.side_effect = side_effect
