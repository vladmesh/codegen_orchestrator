...
async def test_runtime_consumers_resolve_same_slug_dir_and_compose_project():
    ...
    qa_session = QATargetSession(qa_target, AsyncMock())
    assert qa_session.resolve_path("infra/compose.yml") == f"{SERVICE_DIR}/infra/compose.yml"
    assert qa_session.resolve_container(f"{RUNTIME_SLUG}-backend-1") == f"{RUNTIME_SLUG}-backend-1"
