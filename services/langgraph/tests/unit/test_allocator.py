...
class TestEnsureProjectAllocations:
    ...
    @pytest.mark.asyncio
    async def test_new_allocation_calls_atomic_endpoint(self):
        """When no existing allocations, should use allocate_next_port."""
        mock_client = AsyncMock()
        mock_client.get_project_allocations = AsyncMock(return_value=[])
        mock_client.list_servers = AsyncMock(
            return_value=[
                {
                    "handle": "srv-1",
                    "status": "ready",
        ...
        with patch("src.tools.allocator.api_client", mock_client):
            ...
            result = await ensure_project_allocations("proj-1", modules=["backend"])
    ...
    async def test_existing_allocations_returned_as_is(self):
        ...
        mock_client = AsyncMock()
        mock_client.get_project_allocations = AsyncMock(
            return_value=[
                {
                    "server_handle": "srv-1",
                    "port": 8001,
        ...
        with patch("src.tools.allocator.api_client", mock_client):
            ...
            result = await ensure_project_allocations("proj-1")
    ...
    async def test_multiple_modules_allocate_each(self):
        ...
        with patch("src.tools.allocator.api_client", mock_client):
            ...
            result = await ensure_project_allocations("proj-1", modules=["backend", "frontend"])
