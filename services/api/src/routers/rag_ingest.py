"""RAG document ingestion helpers: signature verification, chunking, embeddings, upsert."""
...
def verify_ingest_signature(request: Request, body: bytes) -> None:
...
async def upsert_document(
    ...
    user_id, project_id = resolve_scope_ids(payload, scope)
...
def parse_scope(scope: str) -> RAGScope:
    try:
...
def resolve_scope_ids(payload: RAGDocsIngest, scope: RAGScope) -> tuple[int | None, str | None]:
...
async def validate_payload_targets(db: AsyncSession, payload: RAGDocsIngest) -> None:
    scopes = {parse_scope(doc.scope) for doc in payload.documents}
...
def chunk_document(text: str, encoding: tiktoken.Encoding) -> list[str]:
