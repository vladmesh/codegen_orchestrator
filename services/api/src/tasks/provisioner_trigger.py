    ...
    payload = {
        "input": {
            "messages": [
                {
                    "role": "system",
                    "content": f"Provision server {server_handle}"
                }
            ],
        ...
        "config": {
            "configurable": {
                "thread_id": f"provisioner-{server_handle}"
            }
        }
    }
...
async def provisioner_trigger_worker():
    ...
    logger.info(
        f"Starting Provisioner Trigger Worker (listening on {PROVISIONER_TRIGGER_CHANNEL})"
    )
