...
def _format_project_details(project: dict) -> str:
    ...
    if repo_url:
        lines.append(f"🔗 Репо: {repo_url}")
    if deployed_url:
        lines.append(f"🌐 URL: {deployed_url}")
...
def _format_server_line(server: dict) -> str:
    ...
    return f"{status_emoji} **{handle}** [{status}] — {ip} (RAM: {ram_used}/{ram_total} MB)"
...
async def _handle_project(query, parts: list[str]) -> None:
    ...
    if action == ACTION_DETAILS:
        ...
        if project is None:
            await query.edit_message_text(
                f"⚠️ Проект {project_id} не найден",
                reply_markup=back_to_menu_keyboard(),
            )
            return
