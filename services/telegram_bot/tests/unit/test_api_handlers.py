...
async def test_api_get_constructs_correct_url(mock_settings):
    ...
    with patch("httpx.AsyncClient") as mock_client:
        ...
        mock_get.assert_called_with("http://api:8000/api/projects")

        # Case 2: Path without /
        await _api_get("servers")
        mock_get.assert_called_with("http://api:8000/api/servers")


@pytest.mark.asyncio
async def test_handle_projects_calls_correct_endpoint():
    """Test that _handle_projects calls /projects (not /api/projects)."""
    query = MagicMock()
    ...
    with patch("src.handlers._api_get", new_callable=AsyncMock) as mock_api_get:
        ...
        # THIS IS THE CRITICAL TEST: Verify the path does NOT contain double /api
        mock_api_get.assert_called_once_with("/projects")


@pytest.mark.asyncio
async def test_handle_servers_calls_correct_endpoint():
    """Test that _handle_servers calls /servers (not /api/servers)."""
    query = MagicMock()
    query.edit_message_text = AsyncMock()
    ...
    with patch("src.handlers._api_get", new_callable=AsyncMock) as mock_api_get:
        ...
        mock_api_get.assert_called_once_with("/servers?is_managed=true")
