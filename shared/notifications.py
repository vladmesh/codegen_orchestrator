...
async def send_telegram_message(
    ...
    if not _check_rate_limit(telegram_id):
        logger.warning(
            f"Rate limit exceeded for user {telegram_id}, skipping notification"
        )
        return False
...
async def notify_admins(message: str, level: str = "info") -> int:
    ...
    tasks = [
        send_telegram_message(user["telegram_id"], formatted_message)
    ...
    logger.info(
        f"Notified {success_count}/{len(admin_users)} admins with level={level}"
    )
...
def _check_rate_limit(telegram_id: int) -> bool:
    ...
    _rate_limit_storage[telegram_id] = [
        ts for ts in _rate_limit_storage[telegram_id]
        if ts > cutoff
    ]
