...
def test_production_deploy_provisions_codex_worker_runtime():
    # The codex image is named in the chain the three halves of the worker image
    # release share (publish, pull, record), not repeated in each of them; the pull
    # half is what carries it into production, so it has to read that chain.
