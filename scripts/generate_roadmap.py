...
async def fetch_roadmap_data(
    ...
    async with httpx.AsyncClient(base_url=api_url, timeout=10) as client:
        ...
        for ms in milestones:
            wi_resp = await client.get(f"/api/milestones/{ms['id']}/tasks")
    ...
    return milestones, tasks_by_milestone, unsorted
...
def format_roadmap(
    ...
    """Format milestones and tasks into ROADMAP markdown."""
    ...
    for ms in milestones:
        ...
        if status == "completed":
            ...
            items = tasks_by_milestone.get(ms["id"], [])
            for wi in items:
...
async def main(api_url: str, output: str, project_id: str) -> None:
    milestones, tasks_by_milestone, unsorted = await fetch_roadmap_data(api_url, project_id)
    content = format_roadmap(milestones, tasks_by_milestone, unsorted)
